///    Title: make a synthesized csv file in the pedometer directory
///    Date: 2022-11-17    Sven Ponelat
///
///    Use: To be run inside the pedometer folder
///
///    Change Log:
///    =========================================================
///    2024-02-07   Making it work for nixos
///
...
pub fn convert_from_strings(array: Vec<Vec<String>>) -> Result<Vec<Incoming>, String> {
  ...
  for day in array {
    ...
    if r_date.is_err() {
      let message = format!(
        "The date component of item number {} did not convert!",
        item_counter
      )
        .to_string();
      return Err(message);
    ...
    if r_steps.is_err() {
      let message = format!(
        "The steps component of item number {} did not convert!",
        item_counter
      )
        .to_string();
      return Err(message);
    ...
    if r_dur.is_err() {
      let message = format!(
        "The duration component of item number {} did not convert!",
        item_counter
      )
        .to_string();
      return Err(message);
    ...
    if r_dist.is_err() {
      let message = format!(
        "The distance component of item number {} did not convert!",
        item_counter
      )
        .to_string();
      return Err(message);
    ...
    if r_cal.is_err() {
      let message = format!(
        "The calories component of item number {} did not convert!",
        item_counter
      )
        .to_string();
      return Err(message);
    ...
    if r_floors.is_err() {
      let message = format!(
        "The floors component of item number {} did not convert!",
        item_counter
      )
        .to_string();
      return Err(message);
...
pub fn write_outfile(csv_file: &str, months: &BTreeMap<i64, Basket>) -> Result<(), String> {
  ...
  {
    Err(_) => {
      return Err("Problem opening export to csv file".to_string());
    }
    Ok(file) => file,
  };
  ...
  for key in months {
    ...
    match file.write_all(line.as_bytes()) {
      Err(_) => {
        return Err("Problem writing data csv file".to_string());
      }
      _ => {}
    }
  }
